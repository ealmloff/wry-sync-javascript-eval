...
pub(crate) fn test_call_callback() {
    ...
    extern "C" {
        ...
        fn calls_callback(cb: Box<dyn FnMut(u32) -> u32>, value: u32) -> u32;
    }

    let callback = Box::new(|x: u32| x + 1);
...
pub(crate) fn test_call_callback_async() {
    ...
    extern "C" {
        ...
        fn calls_callback_async(cb: Box<dyn FnMut(u32)>, value: u32);
    }

    let (result_tx, result_rx) = std::sync::mpsc::channel();
    let callback = Box::new(move |x: u32| {
